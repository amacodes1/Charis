        ...
        <div className="flex md:flex-row mx-auto md:mt-3">
            ...
            <div className={`flex-1 md:space-x-2 pb-3 md:block md:pb-0 ${mobileMenuOpen ? 'p-4 md:p-0 block' : 'hidden'}`}>
                <div className="flex md:flex-row flex-col items-center md:mx-auto h-screen md:h-auto ltr mt-2">
                  <Link href="/" className="catLink"
                     onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>Home
                  </Link>
                  <div className="relative inline-block group">
                     <Link href="/"
                         onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="catLink">Categories
                     </Link>
                     <div className="hidden group-hover:block absolute space-y-4 bg-black text-white px-6 py-6 gap-4 w-36 top-full left-16">
                          <Link href="#" className="block text-white hover:text-gray-300">Music</Link>
                          <Link href="#" className="block text-white hover:text-gray-300 ">Movies</Link>
                          <Link href="#" className="block text-white hover:text-gray-300">Sports</Link>
                          <Link href="#" className="block text-white hover:text-gray-300">Politics</Link>
                          <Link href="#" className="block text-white hover:text-gray-300">Food</Link>
                      </div>
                  </div>
                   <Link href="/" className="catLink"
                      onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>About
                    </Link>
                   <Link href="/" className="catLink"
                      onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>Contact
                    </Link>
                </div>
            </div>
            ...
            <div className={`inline-block relative ltr ml-8 md:ml-8 lg:ml-8 flex items-center flex-row max-w-xs w-fit h-fit bg-black rounded-full cursor-pointer overflow-hidden ${mobileMenuOpen ? 'hidden' : 'block'}`}>
                ...
                <input className={`inputBox bg-transparent outline-inherit border-0 w-full py-2 pr-2 text-sm ${!mobileMenuOpen ? 'hidden' : 'block'}`} placeholder="Search Products" id="inputBox" type="text" />
            </div>
        </div>   
    </nav>
  )
